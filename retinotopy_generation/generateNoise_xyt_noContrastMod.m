function moviedata=generateNoise_xyt_noContrastMod(maxSpatFreq,maxTempFreq,contrastSigma,duration);
...
%     
    ...
    imscaled = imscaled - min(imscaled(:));
    imscaled = imscaled/max(imscaled(:));
    moviedata = uint8(floor(imscaled(1:imsize,1:imsize,:)*254)+1);
