...
while goodRect == 0
    subplot(1,2,1)
    imshow(imadjust(input_vid(:,:,1)))
    rect = getrect; subplot(1,2,2); imshow(imcrop(imadjust(input_vid(:,:,1)), rect))
...
sz = size(imcrop(input_vid(:,:,1), rect));
mov = zeros(sz(1), sz(2), size(input_vid, 3), 'uint16');

for i = 1:size(input_vid, 3)
    mov(:,:,i) = imcrop(input_vid(:,:,i), rect);
end
...
for frameNum = START_FRAME:FRAME_JUMP:length(mov)
    ...
    [centroid, area] = getPupilValues(frame, frameNum, centroid, area);
...
% Commenting out the writing of the vid... 
%
...
function [centroid, area] = getPupilValues(frame, frameNum, centroid, area)
global RADIUS_RANGE POLARITY METHOD SENSITIVITY EDGE_THRESH annotatedMov
